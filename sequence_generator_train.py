...
class View:
    ...
    def __init__(self, chromosome_name, bam_file_path, reference_file_path, vcf_path, output_file_path,
        ...
        self.pileup_generator = RegionPileupGenerator(bam_file_path, reference_file_path, vcf_path, chromosome_name)
    ...
    def parse_region(self, start_index, end_index):
        ...
        for i in range(start_index, end_index):
            ...
            # img, allele1, allele2, vcf_a, vcf_b, pos_vals, start_index, end_index = \
            #     self.pileup_generator.create_region_alignment_image(interval_start, interval_end, positional_variants)
            img, training_sequences = \
                self.pileup_generator.get_trainable_image_sequence(interval_start, interval_end, positional_variants)

            filename = self.chromosome_name + '_' + str(interval_start) + '_' + str(interval_end)
            self.pileup_generator.save_image_as_png(img, self.output_dir, filename)
...
def parallel_run(chr_name, bam_file, ref_file, vcf_file, output_dir, conf_intervals, thread_no):
    ...
    view_ob = View(chromosome_name=chr_name,
                   ...
                   thread_no=thread_no)
    # print(len(confidence_intervals))
    # return the results
...
def chromosome_level_parallelization(chr_name, bam_file, ref_file, vcf_file, output_path, max_threads,
    ...
    for i in tqdm(range(0, total_segments, each_chunk_size), file=sys.stdout, ncols=150):
        start_position = i

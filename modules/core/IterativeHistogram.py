...
class IterativeHistogram:
    def __init__(self, start, stop, n_bins, unbounded_upper_bin=False, unbounded_lower_bin=False, include_upper_edge=True):
        ...
        self.stop = stop
        self.n_bins = n_bins
        self.histogram = [0 for i in range(n_bins)]
        self.bin_size = (self.stop - self.start)/self.n_bins
        self.edges = [self.start + self.bin_size*i for i in range(n_bins+1)]
    ...
    def get_bin(self, x):
        # find index of bin by normalizing and centering the value wrt bin edges and add value to that bin
        bin_index = int(math.floor((x - self.start)/self.bin_size))

        if x == self.stop and self.include_upper_edge:
            bin_index = self.n_bins - 1

        if x < self.start and self.unbounded_lower_bin:
            bin_index = 0

        if x > self.stop and self.unbounded_upper_bin:
            bin_index = self.n_bins - 1
    ...
    def get_normalized_histogram(self):
        ...
        normalized_histogram = [value / total for value in self.histogram]
...
if __name__ == "__main__":
    ...
    iterative_histogram.update(-0.5 + 1.0)       # 10
    ...
    iterative_histogram.update(-0.5 + 0.10)      # 2
